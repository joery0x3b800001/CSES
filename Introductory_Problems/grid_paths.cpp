...
//Doesnt need to check for deadend condition
using namespace std;

const int DIR_LEN = 4;
int dr[DIR_LEN] = {-1,0,1,0}; // U, R, D, L
int dc[DIR_LEN] = {0,1,0,-1}; // U, R, D, L
...
int calculatePath(int pathIdx, int curR, int curC) {
	...
	if (p[pathIdx] < 4) {
		...
		if (!onPath[nxtR][nxtC]) ret += calculatePath(pathIdx + 1, nxtR, nxtC);
	}
	else {
...
int main() {
	...
	int startR = 1;
	int startC = 1;
	int result = calculatePath(startIdx, startR, startC);
	cout << result << '\n';
}