...
struct IOPre {
    ...
    std::array<std::pair<int, std::array<char, 4>>, SZ> val;
    constexpr IOPre() : num{}, val{} {
        for (int i = 0; i < SZ; i++) {
            int n = i;
            for (int j = 3; j >= 0; j--) {
        ...
        for (int i = 0; i < SZ; ++i) {
            int j = i;
            int sz = 0;
            while (j) val[i].second[3 - (sz++)] = '0' + (j % 10), j /= 10;
            val[i].first = 4 - sz;  // starting point
        }
    }
};
struct IO {
    ...
    inline std::enable_if_t<std::is_integral_v<T>, void> write_int(T x) {
        ...
        if (x >= TENTHOUSAND) {
            int X = x / TENTHOUSAND;
            int num = io_pre.val[X].first;
            memcpy(std::begin(output_buffer) + output_ptr_right, std::begin(io_pre.val[X].second) + num, 4 - num);
            output_ptr_right += 4 - num;
            memcpy(std::begin(output_buffer) + output_ptr_right, std::begin(io_pre.num) + (x % TENTHOUSAND) * 4, 4);
            output_ptr_right += 4;
        } else {
            int X = x;
            int num = io_pre.val[X].first;
