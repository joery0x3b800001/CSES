...
inline void writeInt(uint64_t n)
{
    uint64_t N = n, rev, count = 0;
    rev = N;
...
uint64_t customer_start[200000];
uint64_t customer_end[200000];
...
int main()
{
    ...
    {
        scanint(customer_start[i]);
        scanint(customer_end[i]);
    }
 
 
    pdqsort(customer_start, customer_start+n);
    pdqsort(customer_end, customer_end+n);
    ...
    {
        ++counter;
        while (customer_end[idx_end] <= customer_start[idx_start])
        {
            ++idx_end;
            --counter;
        }
